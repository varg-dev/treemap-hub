...
#include <treemap-measurements/metrics/AngularDisplacement.h>
...
float AngularDisplacement::compute(const std::vector<LinearizedTree *> & trees, const std::vector<LinearizedBuffer<Rect>> & layouts, std::uint32_t maxId,
...
float AngularDisplacement::computeAverage(const std::vector<LinearizedTree *> & trees, const std::vector<LinearizedBuffer<Rect>> & layouts, std::uint32_t maxId,
              const std::vector<LinearizedBuffer<std::uint32_t>> & index2ids, const std::vector<std::unordered_map<std::uint32_t, std::uint32_t>> & nodeIndexByIds)
{
    if (trees.size() < 2)
    {
    ...
    for (auto i = 1ull; i < trees.size(); ++i)
    {
        ...
        result += angularDisplacement(prevTree, nodeIndexByIds[i-1], layouts[i-1], currentTree, nodeIndexByIds[i], layouts[i], occuringInBothRevisions);
    }
...
float AngularDisplacement::angularDisplacement(LinearizedTree * tree, const std::unordered_map<std::uint32_t, std::uint32_t> & nodeIndexById, const LinearizedBuffer<Rect> & layout,
                          ...
                          const std::vector<std::uint32_t> & occuringInBothRevisions)
{
    ...
    std::tie(other, result) = computeAngularMetrics(tree, nodeIndexById, layout, treeComp, nodeIndexByIdComp, layoutComp, occuringInBothRevisions);
